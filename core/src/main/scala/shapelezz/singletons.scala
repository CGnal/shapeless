...
trait SingletonTypeUtils extends ReprTypes {
  ...
  def fieldTypeCarrier(tpe: Type) =
    mkTypeCarrier(tq"{ type T = $tpe ; type ->>[V] = Field[V] ; type Field[V] = _root_.shapelezz.labelled.FieldType[$tpe,V] }")
...
class SingletonTypeMacros(val c: whitebox.Context) extends SingletonTypeUtils with NatMacroDefns {
  ...
  def mkWitness(sTpe: Type, s: Tree): Tree = {
    q"""
      _root_.shapelezz.Witness.mkWitness[$sTpe]($s.asInstanceOf[$sTpe])
    """
  }
  ...
  def mkOps(sTpe: Type, w: Tree): Tree = {
    ...
    q"""
      {
        final class $name extends _root_.shapelezz.syntax.SingletonOps {
          type T = $sTpe
          val witness = $w
        }
        new $name
  ...
  def materializeWiden[T: WeakTypeTag, Out: WeakTypeTag]: Tree = {
    ...
    if (widenTpe =:= tpe)
      ...
      q"_root_.shapelezz.Widen.instance[$tpe, $widenTpe](x => x)"
  }
}
