...
class LabelledMacros(val c: whitebox.Context) extends SingletonTypeUtils with CaseClassMacros {
  ...
  def mkDefaultSymbolicLabellingImpl[T](implicit tTag: WeakTypeTag[T]): Tree = {
    ...
    val labelsValue =
      labelValues.foldRight(q"_root_.shapelezz.HNil": Tree) {
        case (elem, acc) => q"_root_.shapelezz.::($elem, $acc)"
      }

    q"""
      new _root_.shapelezz.DefaultSymbolicLabelling[$tTpe] {
        type Out = $labelsTpe
        def apply(): $labelsTpe = $labelsValue
      } : _root_.shapelezz.DefaultSymbolicLabelling.Aux[$tTpe, $labelsTpe]
    """
  }
