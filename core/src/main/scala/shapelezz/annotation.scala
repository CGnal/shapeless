...
class AnnotationMacros(val c: whitebox.Context) extends CaseClassMacros {
  ...
  def materializeAnnotation[A: WeakTypeTag, T: WeakTypeTag]: Tree = {
    ...
    annTreeOpt match {
      case Some(annTree) =>
        q"_root_.shapelezz.Annotation.mkAnnotation[$annTpe, $tpe]($annTree)"
      case None =>
  ...
  def materializeAnnotations[A: WeakTypeTag, T: WeakTypeTag, Out: WeakTypeTag]: Tree = {
    ...
    val outTree = wrapTpeTrees.foldRight(q"_root_.shapelezz.HNil": Tree) {
      case ((_, bound), acc) => pq"_root_.shapelezz.::($bound, $acc)"
    }

    q"_root_.shapelezz.Annotations.mkAnnotations[$annTpe, $tpe, $outTpe]($outTree)"
  }
}
