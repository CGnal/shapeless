...
package record {
  ...
  class SelectorMacros(val c: whitebox.Context) extends CaseClassMacros {
    ...
    def applyImpl[L <: HList, K](implicit lTag: WeakTypeTag[L], kTag: WeakTypeTag[K]): Tree = {
      ...
      findField(lTpe, kTpe) match {
        case Some((v, i)) =>
          q"""
            new _root_.shapelezz.ops.record.UnsafeSelector($i).
              asInstanceOf[_root_.shapelezz.ops.record.Selector.Aux[$lTpe, $kTpe, $v]]
          """
        case _ =>
  ...
  class UpdaterMacros(val c: whitebox.Context) extends CaseClassMacros {
    ...
    def applyImpl[L <: HList, F](implicit lTag: WeakTypeTag[L], fTag: WeakTypeTag[F]): Tree = {
      ...
      q"""
        new _root_.shapelezz.ops.record.UnsafeUpdater($i)
          .asInstanceOf[_root_.shapelezz.ops.record.Updater.Aux[$lTpe, $fTpe, $uTpe]]
      """
    }
  }
    ...
    def applyImpl[L <: HList, K, A, B]
      ...
      findField(lTpe, kTpe) match {
        case Some((v, i)) if v <:< aTpe =>
          ...
          q"""
            new _root_.shapelezz.ops.record.UnsafeModifier($i).
              asInstanceOf[_root_.shapelezz.ops.record.Modifier.Aux[$lTpe, $kTpe, $aTpe, $bTpe, $outTpe]]
          """
        case _ =>
          abort(s"No field $kTpe in record type $lTpe")
      }
    ...
    def applyImpl[L <: HList, K](implicit lTag: WeakTypeTag[L], kTag: WeakTypeTag[K]): Tree = {
      ...
      findField(lTpe, kTpe) match {
        case Some((v, i)) =>
          ...
          q"""
            new _root_.shapelezz.ops.record.UnsafeRemover($i).
              asInstanceOf[_root_.shapelezz.ops.record.Remover.Aux[$lTpe, $kTpe, ($v, $outTpe)]]
          """
        case _ =>
          abort(s"No field $kTpe in record type $lTpe")
      }
  ...
  class LacksKeyMacros(val c: whitebox.Context) extends CaseClassMacros {
    ...
    def applyImpl[L <: HList, K](implicit lTag: WeakTypeTag[L], kTag: WeakTypeTag[K]): Tree = {
      ...
      findField(lTpe, kTpe) match {
        case None =>
          q"""
            new _root_.shapelezz.ops.record.LacksKey[$lTpe, $kTpe] {}
          """
        case _ =>
