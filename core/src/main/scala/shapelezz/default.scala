...
import shapelezz.labelled.{ FieldType, field }
...
class DefaultMacros(val c: whitebox.Context) extends CaseClassMacros {
  ...
  def materialize[T: WeakTypeTag, L: WeakTypeTag]: Tree = {
    ...
    // Fixes https://github.com/milessabin/shapelezz/issues/474
    ...
    val resultTree = wrapTpeTrees.foldRight(q"_root_.shapelezz.HNil": Tree) { case ((_, value), acc) =>
      q"_root_.shapelezz.::($value, $acc)"
    }

    q"_root_.shapelezz.Default.mkDefault[$tpe, $resultTpe]($resultTree)"
  }
}
