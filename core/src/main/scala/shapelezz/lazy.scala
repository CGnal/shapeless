...
class LazyMacros(val c: whitebox.Context) extends CaseClassMacros with OpenImplicitMacros with LowPriorityTypes {
  ...
  def mkLazyImpl[I](implicit iTag: WeakTypeTag[I]): Tree =
    mkImpl[I](
      (tree, actualType) => q"_root_.shapelezz.Lazy.apply[$actualType]($tree)",
      q"null.asInstanceOf[_root_.shapelezz.Lazy[_root_.scala.Nothing]]"
    )

  def mkStrictImpl[I](implicit iTag: WeakTypeTag[I]): Tree =
    mkImpl[I](
      (tree, actualType) => q"_root_.shapelezz.Strict.apply[$actualType]($tree)",
      q"null.asInstanceOf[_root_.shapelezz.Strict[_root_.scala.Nothing]]"
    )
