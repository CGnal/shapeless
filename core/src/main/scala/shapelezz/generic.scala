...
import ops.{ hlist, coproduct }
  ...
  * genCat: shapelezz.Generic[Cat]{ type Repr = String :: Int :: HNil } = ...
  *
  * scala> val genDog = Generic[Dog]
  * genDog: shapelezz.Generic[Dog]{ type Repr = String :: Int :: HNil } = ...
  *
  ...
  * genAnimal: shapelezz.Generic[Animal]{ type Repr = Cat :+: Dog :+: CNil } = ...
  *
  ...
  * }}}
  *
  * Inr and Inl are [[shapelezz.Coproduct]] constructors.
  * shapelezz constructs each class representation as a sort of
  * "nested Either" using Coproduct. So in our example, genAnimal would essentially encode garfield as Inl(garfield)
  * and odie as Inr(Inl(odie)). Please see [[shapelezz.Coproduct]] for more details.
  * }}}
  *
...
/**
  ...
  *
  * Continuing the example from [[shapelezz.Generic]], we use LabelledGeneric to convert an object to an [[shapelezz.HList]]:
  *
  * {{{
  * scala> val lgenDog = LabelledGeneric[Dog]
  * lgenDog: shapelezz.LabelledGeneric[Dog]{ type Repr = Record.`'name -> String, 'bonesHidden -> Int`.T } = ...
  *
  ...
  * using [[shapelezz.Witness]] types.
  *
  * As with [[shapelezz.Generic]], the representation for Animal captures the subclass embedding rather than the fields in the class,
  * using [[shapelezz.Coproduct]]:
  *
  * {{{
  * scala> val lgenAnimal = LabelledGeneric[Animal]
  * lgenAnimal: shapelezz.LabelledGeneric[Animal]{ type Repr = Union.`'Cat -> Cat, 'Dog -> Dog`.T } = ...
  *
...
object LabelledGeneric {
  ...
  /** Like [[shapelezz.Generic.Aux]], this is an implementation of the Aux pattern, please
    * see comments there.
    * @tparam T the type
  ...
  /** Provides an instance of LabelledGeneric for the given T. As with [[shapelezz.Generic]],
...
trait ReprTypes {
  ...
  def fieldTypeTpe = typeOf[shapelezz.labelled.FieldType[_, _]].typeConstructor
  def keyTagTpe = typeOf[shapelezz.labelled.KeyTag[_, _]].typeConstructor
...
trait CaseClassMacros extends ReprTypes {
  ...
  def mkHListValue(elems: List[Tree]): Tree =
    elems.foldRight(q"_root_.shapelezz.HNil": Tree) {
      case (elem, acc) => q"_root_.shapelezz.::($elem, $acc)"
    }
  ...
  // See https://github.com/milessabin/shapelezz/issues/212
  ...
  object CtorDtor {
    def apply(tpe: Type): CtorDtor = {
      ...
      def mkCtorDtor0(elems0: List[(TermName, Type)]) = {
        ...
        val reprPattern =
          elems.foldRight(q"_root_.shapelezz.HNil": Tree) {
            case ((bound, _), acc) => pq"_root_.shapelezz.::($bound, $acc)"
          }
        new CtorDtor {
      ...
      def mkCtorDtor1(elems: List[(TermName, TermName, Type)], pattern: Tree, rhs: List[Tree]) = {
        val reprPattern =
          elems.foldRight(q"_root_.shapelezz.HNil": Tree) {
            case ((bound, _, _), acc) => pq"_root_.shapelezz.::($bound, $acc)"
          }
        new CtorDtor {
      ...
      lowerKind(tpe) match {
        ...
        case tpe if tpe =:= typeOf[Unit] =>
          new CtorDtor {
            ...
            def reprBinding: (Tree, List[Tree]) = (pq"_root_.shapelezz.HNil", Nil)
          }
...
class GenericMacros(val c: whitebox.Context) extends CaseClassMacros {
  ...
  def mkProductGeneric(tpe: Type): Tree = {
    ...
    q"""
      final class $clsName extends _root_.shapelezz.Generic[$tpe] {
        type Repr = ${reprTypTree(tpe)}
        def to(p: $tpe): Repr = (p match { case $to }).asInstanceOf[Repr]
        def from(p: Repr): $tpe = p match { case $from }
      }
      new $clsName(): _root_.shapelezz.Generic.Aux[$tpe, ${reprTypTree(tpe)}]
    """
  }
  ...
  def mkCoproductGeneric(tpe: Type): Tree = {
    ...
    val to = {
      ...
      q"""_root_.shapelezz.Coproduct.unsafeMkCoproduct((p: @_root_.scala.unchecked) match { case ..$toCases }, p).asInstanceOf[Repr]"""
    }
    ...
    q"""
      final class $clsName extends _root_.shapelezz.Generic[$tpe] {
        ...
        def from(p: Repr): $tpe = _root_.shapelezz.Coproduct.unsafeGet(p).asInstanceOf[$tpe]
      }
      new $clsName(): _root_.shapelezz.Generic.Aux[$tpe, ${reprTypTree(tpe)}]
    """
  }
  ...
  def mkIsTuple[T: WeakTypeTag]: Tree = {
    ...
    q"""new _root_.shapelezz.IsTuple[$tTpe]: _root_.shapelezz.IsTuple[$tTpe]"""
  }
  ...
  def mkHasProductGeneric[T: WeakTypeTag]: Tree = {
    ...
    q"""new _root_.shapelezz.HasProductGeneric[$tTpe]: _root_.shapelezz.HasProductGeneric[$tTpe]"""
  }
  ...
  def mkHasCoproductGeneric[T: WeakTypeTag]: Tree = {
    ...
    q"""new _root_.shapelezz.HasCoproductGeneric[$tTpe]: _root_.shapelezz.HasCoproductGeneric[$tTpe]"""
  }
}
