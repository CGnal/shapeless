...
class CachedMacros(override val c: whitebox.Context) extends LazyMacros(c) with OpenImplicitMacros {
  ...
  def materializeCached[T: WeakTypeTag]: Tree = {
    ...
    if (CachedMacros.deriving || concurrentLazy) {
      ...
      q"_root_.shapelezz.Cached($tree0)"
    } else {
      ...
      try {
        ...
        treeOpt.getOrElse {
          ...
          // https://github.com/fommil/spray-json-shapelezz/issues/14.
          val tree = mkImpl[T](
            (tree, actualType) => q"_root_.shapelezz.Cached[$actualType]($tree)",
            q"null.asInstanceOf[_root_.shapelezz.Cached[_root_.scala.Nothing]]"
          )
